...
#define L1 5
#define L2 6
#define L3 7    // CHange L3 here 
// How many number of balls to render
#define numBalls 100
#define numBalls1 85
#define numBalls2 5
// numBalls3 = 100 - numBalls1 - numBalls2
...
void RenderManager::SetUpGeometry()
{
    ...
    cout << "L1 = 5 -> NumTriangles = " << sphere1NumPrimitives << endl;
    ...
    cout << "L2 = 6 -> NumTriangles = " << sphere2NumPrimitives << endl;
    ...
    cout << "L3 = 7 -> NumTriangles = " << sphere3NumPrimitives << endl;
...
int main() 
{
    ...
    //b1.distToCamera = (rand() % 7) - 10; // random from -4 to -10 (Furthest from camera)
    //b1.distToCamera = (rand() % 7) - 3; // random from -3 to 3 (Middle range from camera)
    //b1.distToCamera = (rand() % 7) + 4; // random from 4 to 10 (Closest to camera)
    ...
    for (int i = 0; i < numBalls; i++) {
        ...
        double rng1 = (rand() % 30) + 10;
        b1.BP.h0 = rng1;
        b1.BP.hmax = rng1;
        b1.BP.h = rng1;
        b1.BP.t_last = -1 * sqrt(2*b1.BP.h0/9.8);
        b1.BP.vmax = sqrt(2*b1.BP.hmax*9.8);
...
UpdateBallPhysics(Ball& ball){
    ...
    if (ball_physics.hmax > ball_physics.hstop){
        ...
        //TODO: ADD CODE!
        double rng = (rand() % 30) + 10;
        ball.BP.h0 = rng;
        ball.BP.hmax = rng;
        ball.BP.h = rng;
        ball.BP.t_last = -1 * sqrt(2*ball.BP.h0/9.8);
        ball.BP.vmax = sqrt(2*ball.BP.hmax*9.8);
    }
//
//    cout << "Current height: " << ball.BP.h << endl;
}
...
BounceBall(std::vector<Ball> &balls, RenderManager &rm, glm::vec3 camPos)
{
    ...
    for (int i = 0; i < balls.size(); i++) {
        ...
        if (i < numBalls1) {
            glm::mat4 M1T = TranslateMatrix(balls[i].x, balls[i].BP.h, balls[i].distToCamera);
            rm.SetColor(0.6, 0.4, 0.0);
            rm.Render(RenderManager::SPHERE1, M1T);
        }
        else if (i < numBalls2) {
            glm::mat4 M1T = TranslateMatrix(balls[i].x, balls[i].BP.h, balls[i].distToCamera);
            rm.SetColor(0.6, 0.4, 0.0);
            rm.Render(RenderManager::SPHERE2, M1T);
        }
        else {
            glm::mat4 M1T = TranslateMatrix(balls[i].x, balls[i].BP.h, balls[i].distToCamera);
            rm.SetColor(0.6, 0.4, 0.0);
            rm.Render(RenderManager::SPHERE3, M1T);
        }
    }
}
