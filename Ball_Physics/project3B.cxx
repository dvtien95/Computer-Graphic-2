...
//        Keep moving in the direction we should go
    ...
    double getDistance(Ball &s)
    {
        ...
        return sqrt( pow(s.pos[0] - pos[0], 2) + pow(s.pos[1] - pos[1], 2) + pow(s.pos[2] - pos[2], 2) );
    }
    ...
    void collideDir(double pos1[3], double dir1[3], double pos2[3], double dir2[3])
    {
        ...
        n[1] = pos1[1] - pos2[1];
        n[2] = pos1[2] - pos2[2];
        double newPosLengthSqrt = sqrt(pow(n[0],2) + pow(n[1],2) + pow(n[2],2));
        n[0] /= newPosLengthSqrt; n[1] /= newPosLengthSqrt; n[2] /= newPosLengthSqrt;
        
    	// 2. Calculate the relative velocity
    	//     vRelative = dir1-dir2
        double vRelative[3] = {0};
        vRelative[0] = dir1[0] - dir2[0];
        vRelative[1] = dir1[1] - dir2[1];
        vRelative[2] = dir1[2] - dir2[2];
//        double vRelativeLengthSqrt = sqrt(pow(vRelative[0],2) + pow(vRelative[1],2) + pow(vRelative[2],2));
//        vRelative[0] /= vRelativeLengthSqrt; vRelative[1] /= vRelativeLengthSqrt; vRelative[2] /= vRelativeLengthSqrt;
        
    	// 3. Calculate the relative along the normal
    	//     vNormal = (vRelative (.) n ) * n  [where (.) is the dot operator]
        double dotVRelativeAndN = vRelative[0]*n[0] + vRelative[1]*n[1] + vRelative[2]*n[2];
        double vNormal[3] = {0};
        vNormal[0] = dotVRelativeAndN * n[0];
        vNormal[1] = dotVRelativeAndN * n[1];
        vNormal[2] = dotVRelativeAndN * n[2];
//        double vNormalLengthSqrt = sqrt(pow(vNormal[0],2) + pow(vNormal[1],2) + pow(vNormal[2],2));
//        vNormal[0] /= vNormalLengthSqrt; vNormal[1] /= vNormalLengthSqrt; vNormal[2] /= vNormalLengthSqrt;
  ...
  while (!glfwWindowShouldClose(window)) 
  {
    if(TICK_LIMIT != 0)
    {
      ...
      for (int i = 0 ; i < numBalls ; i++)
      {
          ...
          {
              bool flag = ballList[i].LookForCollision(ballList[j]);
              
              if (flag == true) {
                  ballList[i].color[0]    +=    0.7    *    (1-ballList[i].color[0]);
                  ballList[i].color[1]    -=    0.1    *    ballList[i].color[1];
                  ballList[i].color[2]    -=    0.1    *    ballList[i].color[2];
                  
//                  ballList[j].color[0]    +=    0.7    *    (1-ballList[j].color[0]);
//                  ballList[j].color[1]    -=    0.1    *    ballList[j].color[1];
//                  ballList[j].color[2]    -=    0.1    *    ballList[j].color[2];
                  
                  ballList[i].tSinceAccident = 0;
//                  ballList[j].tSinceAccident = 0;
                  
                  ballList[i].radius    -=    0.25    *    ballList[i].radius;
              }
              else {
